...
void best_fit(char process_id[3], int space_requested)
{
    ...
    //this loop will find the smallest avaliable space that can store the process
    while (temp->next != NULL)
    {
    ...
    temp = head;
    //this loop will place the new process in the space
...
void worst_fit(char process_id[3], int space_requested)
{
    ...
    //this loop will find the largest possible avaliable space
    while (temp->next != NULL)
    {
...
void compact()
{
    //finding number of unused nodes 
    int num_unused = 1; 
    temp = head;
    while(temp->next!=NULL)
    {
    ...
    while(num_unused!=1)
    {
        ...
        while(temp->next!=NULL)
        {
            if(strcmp(temp->next->process_id,"Unused")==0)
            {
                int space = temp->next->next->start_address - temp->next->next->end_address;
                 temp->next->next->start_address = temp->end_address + 1 ;
                 temp->next->next->end_address = temp->next->next->start_address + space ;
                 left_over_space +=temp->next->available_space;
        ...
        while(temp->next!=NULL)
        {
            ...
            {
                temp->next->start_address = temp->next->start_address - left_over_space;
                temp->next->available_space = temp->next->available_space + left_over_space;
            }
            temp = temp->next;
        }
